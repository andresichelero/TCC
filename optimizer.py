...
def run_optimization_pipeline():
    ...
    all_results = []
    pbar = tqdm(
        enumerate(knn_configurations), total=total_runs, desc="Otimizando Configs KNN"
    )

    for i, config in pbar:
        start_run_time = time.time()
        config_str = ", ".join([f"{k}={v}" for k, v in config.items()])
        pbar.set_description(f"Executando: {config_str}")
        fitness_function_for_bda = create_fitness_function(
            config, X_train_feat, y_train
        )
...
def generate_reports_and_plots(
    ...
    df = df_sorted_by_fitness
    best_config_row = df.iloc[0]
    # Reconstrói o dicionário de parâmetros para exibição, incluindo os novos
    best_knn_params = {
    ...
    try:
        # Usa o dataframe ordenado por ID da configuração para manter a ordem da matriz
        if not df_sorted_by_id["best_solution_vector"].empty:
            ...
            for i in range(num_solutions):
                for j in range(i, num_solutions):
                    ...
                    jaccard_matrix[j, i] = score  # Matriz é simétrica
    ...
    g = sns.relplot(
        ...
        style="metric",
        size="weights",
        col="algorithm",  # <-- cria um sub-gráfico por algoritmo
        col_wrap=2,  # Quebra a linha a cada 2 sub-gráficos para não ficar muito largo
        height=6,
        aspect=1.2,
        palette="bright",
    )
    ...
    for i in range(top_n):
        row = df.iloc[i]
        # Constrói um rótulo mais completo para a legenda
        params_to_show = {
            k: row.get(k)
